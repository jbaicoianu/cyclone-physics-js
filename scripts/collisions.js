...
//console.log(intersection, thispos, v, otherpos, r, obj1, obj2);
          ...
          var contact = new DynamicContact({
            normal: normal,
  ...
  static box_sphere = function() {
    ...
    var center = new Vector3(),      // center of sphere, box-space coordinates
        centerWorld = new Vector3(), // center of sphere, world-space coordinates
        diff = new Vector3(),
        closest = new Vector3(),
        scale = new Vector3(),
        scaledmin = new Vector3(),
        scaledmax = new Vector3();
...
//console.log(axis.toArray(), oneProject, twoProject, distance, oneProject + twoProject - distance);
    ...
    function fillPointFaceBoxBox(box1, box2, toCenter, best, penetration) {
      var point = new Vector3(); // allocate point
      var normal = new Vector3(0,1,0); // allocate normal
    ...
    return function(cylinder, sphere, contacts, dt) {
      ...
      if (spherepos.y - cylinder.offset.y > -halfh && spherepos.y - cylinder.offset.y < halfh) {
        ...
        if (d < cylinder.radius) {
          ...
          var normal = new Vector3().subVectors(capline, spherepos); // allocate normal
...
//      }
  ...
  static capsule_capsule = function() {
    ...
    return function(capsule1, capsule2, contacts, dt) {
      ...
      if (distSquared <= Math.pow(capsule1ScaledRadius + capsule2ScaledRadius, 2)) {
        ...
        let normal = new Vector3().subVectors(p2, p1),
            point = p1.clone();
  ...
  static triangle_sphere = function() {
    ...
    const sphereClosestPointToPlane = new Vector3(),
          ...
          scaledVelocity = new Vector3(),
          intersectionPoint = new Vector3(),
          triangleClosestPoint = new Vector3();
  ...
  static triangle_capsule = function() {
    ...
    const capsuleLine = new Vector3(),
          ...
          intersectionPoint = new Vector3(),
          //normal = new Vector3(),
          capsuleNormal = new Vector3(),
          sphereCenter = new Vector3();
    var localSphere;// = new RigidBody();
    ...
    return function(mesh, sphere, contacts, dt) {
      ...
      if (localcontacts.length > 0) {
        ...
        for (let i = 0; i < localcontacts.length; i++) {
          ...
          if (contact instanceof DynamicContact) {
  ...
  static closest_point_on_triangle = (function() {
    ...
    return function(point, a, b, c, closest) {
      if (!closest) closest = new Vector3(); // Allocate return vector if reference isn't passed in
...
//console.log('p is in face', point, denom, v, w);
  ...
  static distancesquared_between_lines = function() {
  ...
  static ray_sphere = (function() {
    ...
    return function(raypos, dir, spherepos, radius, point) {
      ...
      t = -b - Math.sqrt(discr);
      if (t < 0) t = 0;
      if (!point) point = new Vector3(); // Allocate return vector if reference isn't passed in
...
class SphereCollider {
  constructor(body, args={}) {
    ...
    this.trigger = elation.any(args.trigger, false);

    if (!(this.scale.isVector3)) this.scale = new Vector3().copy(args.scale);
    if (this.offset !== false && !(this.offset.isVector3)) this.offset = new Vector3().copy(args.offset);
  }
  ...
  getInertialMoment() {
    ...
    this.momentInverse = new Matrix4();
  ...
  toJSON() {
    return {
      ...
      offset: {x: elation.any(this.offset.x, 0), y: elation.any(this.offset.y, 0), z: elation.any(this.offset.z, 0)},
...
class PlaneCollider {
  constructor(body, args={}) {
    this.type = 'plane';
    this.body = body;
    this.normal = args.normal || new Vector3(0,1,0);
...
class BoxCollider {
  constructor(body, args={}) {
    this.type = 'box';
    this.body = body;
    this.min = args.min || new Vector3(0,0,0);
    this.max = args.max || new Vector3(0,0,0);
    this.trigger = elation.any(args.trigger, false);

    if (!(this.min.isVector3)) this.min = new Vector3().copy(args.min);
    if (!(this.max.isVector3)) this.max = new Vector3().copy(args.max);

    this.halfsize = new Vector3().copy(this.max).sub(this.min).divideScalar(2);
    this.offset = new Vector3().copy(this.max).add(this.min).divideScalar(2);
  }
  getContacts(other, contacts, dt) {
    if (!contacts) contacts = [];
...
class CylinderCollider {
  constructor(body, args={}) {
    ...
    this.offset = args.offset || new Vector3();
    this.trigger = elation.any(args.trigger, false);

    if (!(this.offset.isVector3)) this.offset = new Vector3().copy(args.offset);
  }
  getContacts(other, contacts, dt) {
    if (!contacts) contacts = [];
    if (other instanceof PlaneCollider) {
...
class MeshCollider {
  constructor(body, args={}) {
    this.type = 'mesh';
    this.body = body;
    this.isroot = elation.any(args.isroot, true);
    ...
    this.boundingSphere = new SphereCollider(body, {});
      ...
      if (this.modeldata.index) {
        ...
        for (var i = 0; i < idxarr.length / 3; i++) {
          let offset = i * 3,
              ...
              p1 = new Vector3((posarr[v1]), (posarr[v1 + 1]), (posarr[v1 + 2])),
              p2 = new Vector3((posarr[v2]), (posarr[v2 + 1]), (posarr[v2 + 2])),
              p3 = new Vector3((posarr[v3]), (posarr[v3 + 1]), (posarr[v3 + 2])),
...
/*
              ...
              p3 = new Vector3((posarr[v3]), (posarr[v3 + 1]), (posarr[v3 + 2])),
*/
              triangle = new TriangleCollider(this.body, [p1, p2, p3]);
        ...
        for (var i = 0; i < posarr.length / 9; i++) {
          let offset = i * 3 * 3,
              p1 = new Vector3(posarr[offset    ], posarr[offset + 1], posarr[offset + 2]),
              p2 = new Vector3(posarr[offset + 3], posarr[offset + 4], posarr[offset + 5]),
              p3 = new Vector3(posarr[offset + 6], posarr[offset + 7], posarr[offset + 8]),
              triangle = new TriangleCollider(this.body, [p1, p2, p3]);
...
*/
  ...
  distanceTo = (function() {
    let triangleClosestPoint = new Vector3();
    return function(point) {
      //return this.normal.dot(point) + this.offset;

...
/**
 * BufferGroup for WebGPU-based physics simulation
 */
...
/**
* GPUPhysicsProcessor - processes all physics on the CPU
* @extends PhysicsProcessor
*/

class GPUPhysicsProcessor extends PhysicsProcessor {
  stagingBuffers = [];
  readBuffers = [];
...
console.log('GO START IT');
  ...
  createBuffers(objects, mapped=false, usage) {
    return new GPUPhysicsBufferGroup(this.device, objects.length, mapped, usage);
  }
  fillBuffers(buffers, objects) {
  }
  update(objects, t, active) {
